...
public interface RecommendMapper {
	...
	// 옷추천(상,하,액) 한 번에 촤라락
	@Select("SELECT * FROM view_drank_mcv WHERE wdate=#{wdate} AND (catetype='상의' OR catetype='하의' OR catetype='액세서리'  ) AND id=#{id} AND rank=1")
	...
	// 옷추천 (하의)
	@Select("SELECT * FROM view_drank_mcv WHERE wdate=#{wdate} AND (catetype='하의') AND id=#{id} AND rank=1")
	public Map<String, Object> clothesRecommendBottom(Map<String, Object> map);
	
	
	// 옷추천 (신발)
	@Select("SELECT * FROM view_drank_mcv WHERE wdate=#{wdate} AND (catetype='신발') AND id=#{id} AND rank=1")
